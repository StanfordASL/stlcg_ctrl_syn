...
class HJIValueFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return  torch.cat([torch.cat([deriv_interp[i](points) for i in range(4)], -1), torch.zeros_like(deriv_interp[0](points))], dim=-1)  * grad_output
...
class HJIValueFunction_cuda(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return  torch.cat([torch.cat([deriv_interp_cuda[i](points) for i in range(4)], -1), torch.zeros_like(deriv_interp_cuda[0](points))], dim=-1) * grad_output
...
def get_complete_index_batch(signal, formula, formula_input, absolute_threshold, time_dim=1):
    trace = formula(formula_input(signal)).squeeze(-1)
    q = torch.where(trace >= 0)
    qq = torch.cat([q[0][:1]-1, q[0]], dim=0)
    idx = (qq[1:] - qq[:-1]).bool()
    tmp = absolute_threshold * torch.ones(trace.shape[0]).to(trace.device)
...
def get_hji_time(traj, cov_idx, coverage_threshold, tmax=3.5, dt=0.05, time_dim=1, stay_len=4):
    bs = traj.shape[0]
    hji_time = torch.arange(0, tmax+dt, dt).unsqueeze(0).repeat(bs,1).to(traj.device)
    time_remaining = traj.shape[time_dim] - cov_idx - 4
    gather = torch.tensor([[ci for ci in range(cov_idx[i] + stay_len)] + [coverage_threshold + gi for gi in range(time_remaining[i])] for i in range(len(cov_idx))]).to(traj.device).clamp_max(hji_time.shape[1]-1)
...
class STLPolicy(torch.nn.Module):
    ...
    def safety_preserving_loss(self, complete_traj, us, formula, formula_input, absolute_threshold=coverage_threshold, eps=1E-3):
        # for states V > -eps and have hamiltonians < 0, want to increase those > 0 
        H = self.hamiltonian(complete_traj, us, formula, formula_input, absolute_threshold=absolute_threshold)
        value = self.HJI_value(complete_traj, formula, formula_input, absolute_threshold=absolute_threshold)[:,:-1,:]
    ...
    def adversarial_safety_preserving_loss(self, complete_traj, us, formula, formula_input, absolute_threshold=coverage_threshold, eps=1E-3):
        H = self.hamiltonian(complete_traj, us, formula, formula_input, absolute_threshold=absolute_threshold)
        value = self.HJI_value(complete_traj, formula, formula_input, absolute_threshold=absolute_threshold)[:,:-1,:]
        boundary = H[value > -eps]
        return boundary.mean()
    ...
    def HJI_loss(self, x_traj, formula, formula_input, absolute_threshold=coverage_threshold):
        ...
        '''
        return self.HJI_value(x_traj, formula, formula_input).squeeze(-1).relu().sum(1).mean() * self.dt
    ...
    def hamiltonian(self, complete_traj, us, formula, formula_input, absolute_threshold=coverage_threshold, lr=0.7, lf=0.5):
